...
if is_flash_attn_2_available():
    ...
    from flash_attn.bert_padding import (  # noqa: PGH004
        index_first_axis,
        pad_input,
        unpad_input,
    )
...
class MoEV2CausalLMOutputWithPast(ModelOutput):
    ...
    Args:
        ...
        router_logits (`tuple(torch.FloatTensor)`, *optional*,
            returned when `output_router_logits=True` is passed or when `config.add_router_probs=True`):
...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    warnings.warn(
        "Calling `transformers.models.BailingMoeV2.modeling_BailingMoeV2._prepare_4d_attention_mask` "
        "is deprecated and will be removed in v4.37. "
...
def _make_causal_mask(
    ...
    warnings.warn(
        "Calling `transformers.models.BailingMoeV2.modeling_BailingMoeV2._make_causal_mask` is deprecated and "
        "will be removed in v4.37."
        "Use `transformers.models.BailingMoeV2.modeling_BailingMoeV2.AttentionMaskConverter._make_causal_mask",
        stacklevel=2,
    )
...
class BailingMoeV2RotaryEmbedding(nn.Module):
    def __init__(self, config: BailingMoeV2Config, device=None):
        ...
        # Ensure rope_scaling has 'factor' if not present (required by newer transformers)
        if (
            hasattr(self.config, "rope_scaling")
            and self.config.rope_scaling is not None
            and "factor" not in self.config.rope_scaling
        ):
            self.config.rope_scaling["factor"] = 1.0

        if self.rope_type not in ROPE_INIT_FUNCTIONS:
            for candidate in ("default", "base", "no_scaling"):
                if candidate in ROPE_INIT_FUNCTIONS:
                    self.rope_type = candidate
                    break
            else:
                self.rope_type = next(iter(ROPE_INIT_FUNCTIONS))
...
class BailingMoeV2Attention(nn.Module):
    ...
    def __init__(self, config: BailingMoeV2Config, layer_idx: Optional[int] = None):
        ...
        if self.config.use_qk_norm:
            self.query_layernorm = BailingMoeV2RMSNorm(
...
class BailingMoeV2FlashAttention2(BailingMoeV2Attention):
    """
    BailingMoeV2 flash attention module. This module inherits from `BailingMoeV2Attention`
    as the weights of the module stays untouched.
    The only required change would be on the forward pass where it needs to correctly call the public API of
    ...
    def _flash_attention_forward(
        ...
        if not self._flash_attn_uses_top_left_mask:
            ...
            # For details, please see the comment in BailingMoeV2FlashAttention2 __init__.
...
class BailingMoeV2SdpaAttention(BailingMoeV2Attention):
    ...
    `BailingMoeV2Attention` as the weights of the module stays untouched.
    ...
    def forward(
        ...
        **kwargs,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "BailingMoeV2Model is using BailingMoeV2SdpaAttention, but "
...
class BailingMoeV2MTPLayer(nn.Module):
    def __init__(self, config: BailingMoeV2Config, layer_idx: int):
        ...
        self.final_layernorm = BailingMoeV2RMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
...
class BailingMoeV2Model(BailingMoeV2PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        layers = (
            self.layers[: -self.num_nextn_predict_layers]
...
class BailingMoeV2ForCausalLM(BailingMoeV2PreTrainedModel, GenerationMixin):
    ...
    @add_start_docstrings_to_model_forward(BAILINGMOEV2_INPUTS_DOCSTRING)
    @replace_return_docstrings(
        output_type=MoEV2CausalLMOutputWithPast, config_class=_CONFIG_FOR_DOC
    )
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        ...
        if self.num_nextn_predict_layers > 0:
            ...
            for i in range(self.num_nextn_predict_layers):
                ...
                if labels is not None:
                    ...
                    shift_labels_mtp, _ = roll_tensor(
                        shift_labels_mtp, shifts=-1, dims=-1, fill_value=-100
                    )
                    mtp_logits_ = mtp_logits.view(-1, self.config.vocab_size)
                    mtp_loss = self.loss_function(
                        mtp_logits_,
                        shift_labels_mtp.to(mtp_logits_.device).view(-1),
